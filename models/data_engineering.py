...
def separate_by_stock(use_roberta=False):
    if use_roberta:
        ...
        features = ['frob_neg', 'frob_neu', 'frob_pos', 'frob_comp', 'Open', 'High', 'Low', 'Close', 'Volume', 'Dividends', 'Stock Splits']
        sentiment_column = 'frob_comp'
    else:
        df = pd.read_csv('../data/complete_next_open.csv')
        features = ['finvader_neg', 'finvader_neu', 'finvader_pos', 'finvader_tot', 'Open', 'High', 'Low', 'Close', 'Volume', 'Dividends', 'Stock Splits']
        sentiment_column = 'finvader_tot'
        
    df['Market Date'] = pd.to_datetime(df['Market Date'])
    
    # Create overall sentiment column
    df['overall_sen'] = df[sentiment_column].apply(_overall_sentiment)
